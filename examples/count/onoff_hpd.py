...
def text_phys(x, y, s, fontdict=None, **kwargs):
    ...
    axes instance (e.g., from subplot or gca).  text_phys just saves the user
...
text_phys(.15, .8, r'$N_{\rm on} = %i,   T_{\rm on} = %4.1f$' % (n_on, T_on), fontsize=14)
text_phys(.15, .75, r'$N_{\rm off} = %i,   T_{\rm off} = %4.1f$' % (n_off, T_off), fontsize=14)
text_phys(.15, .65, r'2nd case has $(N_{\rm off},T_{\rm off})\times$%-4.0f' % bgfac, fontsize=14)
...
line = r'$\hbox{Prior}\; s_{\rm max} = %4.1f \rightarrow B = %4.1f, %4.1f$' %\
       (s_max, bfac, bfac2)
text_phys(.15, .55, line, fontsize=14)
